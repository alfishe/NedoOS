...
#ifdef TARGET_SCRIPT
  _curaddr = _curaddr + 1; //слово - это €чейка скрипта
#else
...
PROC asmlong(LONG value) //разна€ разр€дность LONG/DL
{
  ...
  /*IF (_asms) fwrite((PBYTE)&value, 4, 1, _fout);*/ //fast version (little endian) (не _SIZEOF_LONG, т.к. тут надо размер дл€ таргета!)
