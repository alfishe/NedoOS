...
;*********************** OS_PLAYCOVOX **********************
;Синхронно проигрывает 8-битный PCM-сэмпл через Covox (порт 0xFB, ?Pentagon CovoxFB?).
;Работает на ATM2 и Evo (и других конфигурациях с Covox на порту 0xFB). На ATM3 и PE26
;аппаратного Covox нет ? вызов не приведёт к ошибке, но звука не будет.
;Вызов блокирующий: управление возвращается только после окончания сэмпла.
;На время проигрывания прерывания запрещены (DI), турбо-режим EGA временно выключается
;(бит 3 в порту 0xBD77 сбрасывается) и восстанавливается по завершении.
;    Все аргументы в регистрах:
;        HL - адрес начала сэмпла в области 0xC000..0xFFFF (старший байт адреса >= 0xC0).
;             Сэмпл ? последовательность байтов без знака, центрированных около 0x80
;             (тишина = 0x80, амплитуда 0..255). Конец сэмпла отмечается байтом 0x00.
;             ВАЖНО: 0x00 ? одновременно и ?тишина?, и маркер конца! Значение 0x00
;             внутри сэмпла обрывает проигрывание. При подготовке данных нулевые отсчёты
;             заменяют на 0x01 (см. утилиту evoresc, makesample.h).
;        DE - указатель на таблицу страниц (pagetable) в основной странице процесса
;             (адреса 0x0000+). Таблица ? массив номеров страниц ОЗУ, по одному байту
;             на каждые 16 КБ данных сэмпла. Первый байт ? страница для области 0xC000,
;             второй ? для следующих 16 КБ (после переполнения HL за 0xFFFF), и т.д.
;             Номера страниц ? те же, что возвращает OS_NEWPAGE / OS_GETMAINPAGES
;             (с учётом модели памяти ATM2/ATM3/Evo).
;             Ядро отображает основную страницу процесса в 0x8000 и читает таблицу по
;             адресу DE|0x8000 (устанавливает бит 7 регистра D).
;        HX - задержка между отсчётами (чем меньше ? тем выше частота дискретизации).
;             Типичные значения (реальная частота при 14 МГц):
;                 HX=27  ~  8 кГц
;                 HX=18  ~ 11 кГц  (11025 Гц в WAV)
;                 HX=11  ~ 16 кГц
;                 HX=7   ~ 22 кГц  (22050 Гц в WAV)
;                 HX=3   ~ 33 кГц
;                 HX=1   ~ 44 кГц  (44100 Гц в WAV)
;    Возвращаемых значений нет.
;    Портит все регистры.
;
;Формат каталога сэмплов (как в играх на evosdk, страница SND_PAGE):
;    SMP_COUNT  ? байт: количество сэмплов (до 256).
;    SMP_LIST   ? таблица 4?256 байт, для сэмпла с индексом N (0..SMP_COUNT-1):
;        [SMP_LIST + N + 0]   ? младший байт смещения данных внутри страницы
;        [SMP_LIST + N + 256] ? старший байт смещения (к адресу добавляется 0xC000)
;        [SMP_LIST + N + 512] ? индекс первой страницы в pagetable (~номер_страницы)
;        [SMP_LIST + N + 768] ? значение HX (задержка / частота)
;    Pagetable (массив tpags[256] в основной странице) заполняется при загрузке ресурсов:
;    tpags[i] = номер страницы ОЗУ, куда загружен i-й блок данных (page_N.bin).
;    Для сэмпла, занимающего несколько страниц, в pagetable подряд идут номера
;    последовательных страниц, начиная с индекса из SMP_LIST+512.
;
;Пример проигрывания сэмпла по номеру (упрощённо, из games/_sdk/lib_sound.asm):
;        LD A,SND_PAGE          ;страница со звуковыми данными
;        SETPGC000
;        LD A,(SMP_COUNT|0xC000)
;        LD E,A
;        LD A,L                 ;L = номер сэмпла
;        CP E
;        JR NC,.skip
;        LD H,HIGH(SMP_LIST|0xC000)
;        LD E,(HL)              ;смещение, младший байт
;        INC H
;        LD A,(HL)              ;смещение, старший байт
;        OR 0xC0
;        LD D,A
;        INC H
;        LD A,(HL)              ;индекс в pagetable
;        CPL
;        INC H
;        LD H,(HL)              ;H = задержка
;        EX DE,HL               ;HL = адрес сэмпла, D = задержка
;        LD HX,D                ;HX = задержка
;        LD E,A                 ;E = индекс в pagetable
;        LD D,TPAGES/256        ;D = старший байт адреса pagetable
;        OS_PLAYCOVOX
;.skip
;
;Подготовка WAV-файлов: утилита src/games/_sdk/tools_src/evoresc (ключ -samplelist).
;Конвертирует моно/стерео PCM 8/16 бит в формат Covox, упаковывает в page_*.bin
;и заполняет SMP_LIST / tpags. Готовый sound.bin подключается к проекту как SND_PAGE.
;
...
;коротких синхронных сэмплов (выстрел, взрыв, щелчок и т.п.).
	...
	CALLBDOS
        endm
        ELSE
        ; ESPNET UART coprocessor: same registers as WIZNET (L=sock, HL=-1 error).
        ; include "../_sdk/espnet.asm" in the app. Call OS_ESPINIT once.
        macro OS_ESPINIT
        call esp_init
        endm
        macro OS_NETSOCKET
...
;*********************** OS_GETPAL **********************
;Читает 32-байтную палитру DDp задачи, у которой сейчас фокус видеовывода.
;Не переключает фокус и не меняет палитру вызывающего процесса.
;Формат тот же, что у OS_SETPAL: 16 цветов по 2 байта %grbG11RB (inverted).
;    Все аргументы в регистрах:
;        DE - адрес буфера 32 байта (должен быть доступен в текущей карте страниц)
;    Возвращаемых значений в регистрах нет; данные копируются в буфер DE.
;
;Пример:
;        ld de,mypal
;        OS_GETPAL
        macro OS_GETPAL ;de=palette (32 bytes), from focus task
        ld c,CMD_GETPAL
	CALLBDOS_NOPARAM_A
        endm

;*********************** OS_GETGFX **********************
;Считывает видеосостояние задачи в фокусе. Не захватывает фокус и не меняет
;видеорежим (в отличие от OS_SETGFX).
;D,E,H,L - системные страницы CRT (pgscr*), их можно SETPG. При keep-private
;у задачи свои копии app.scr*; этот вызов их не возвращает.
;    Аргументов нет.
;    Возвращаемые значения в регистрах:
;        A - сырой байт видеорежима (порт #BD77)
;            класс = A&7: 0=EGA 320x200, 2=MC 640x200, 3=6912, 6=текст 80x25
;        B - id задачи в фокусе
;        C - видимый экран 0 или 1 (как OS_SETSCREEN)
;        D,E - страницы экрана 0 (младшая, старшая)
;        H,L - страницы экрана 1 (младшая, старшая)
;
;Пример:
;        OS_GETGFX
;        and 7
;        jr z,is_ega
        macro OS_GETGFX ;out: a=gfxmode, b=focus id, c=screen 0/1, de=scr0 pages, hl=scr1 pages
        ld c,CMD_GETGFX
	CALLBDOS_NOPARAM_A
        endm

;*********************** OS_PUTKEY **********************
;Кладёт код клавиши в ту же очередь, из которой фокусная задача читает OS_GETKEY.
;Не эмулирует матрицу клавиатуры (IN #FE): программы, читающие порты напрямую,
;эту клавишу не увидят.
;Если D=2, вместо очереди ставится запрос на переключение графических задач -
;то же действие, что Symbol Shift+Enter в прерывании ядра
;(PS/2 / PC: Right Shift+Enter).
;    Все аргументы в регистрах:
;        E - код клавиши (sysdefs.asm, секция Usable key codes)
;        D - 0 = символ (буква, цифра, пробел, кириллица CP866)
;            1 = управляющая (Enter, стрелки, key_csenter, F1..F10)
;            2 = переключить графическую задачу (E игнорируется)
;    Возвращаемые значения в регистрах:
;        A - 0 = успешно, 1 = очередь заполнена (при D=2 всегда A=0)
;
;Пример ввода буквы и переключения задачи:
;        ld e,'A'
;        ld d,0
;        OS_PUTKEY
;        ...
;        ld d,2
;        OS_PUTKEY
        macro OS_PUTKEY ;de=key: e=code, d=0 letter / 1 control / 2=SS+Enter next gfx task ;out: a=0 ok, a=1 full
        ld c,CMD_PUTKEY
	CALLBDOS_NOPARAM_A
        endm
