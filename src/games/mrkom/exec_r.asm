...
locations_table:
				...
				dw location_B154
				dw location_B162
				dw location_B168 ;search for archive records
				dw location_B1EE
				dw location_B2E8
				dw location_B315
				dw location_B382
...
location_AE7E:				;interrogation controller
...
loc_AFC3:
				...
				jp      loc_ABA8
;===
location_AFCC:	;wall street tower
                ld      a, (temp_location_id_byte_b9ea+1)
...
location_B549:		;call another person
                call    sub_B595
                ex      de, hl
                ld      a, (temp_location_id_byte_b9ea)
                cp      1Ah
...
deduction:
				...
				ld      a, 3
				ld      (symbol_color), a
				ld      hl, 15*40+20+0x2000+0xc000		;0C4DDh
				ld      (text_screen_address), hl
						ld      a,(characters_in_text_line)
						push af
						ld a,45
...
.loop:
				...
				ld      (byte_AF91), a
				ld      l, a
							;				ld      h, 0x14			;	37h ; '7' vertical offset 8 pix
							;				add     hl, hl
							;				add     hl, hl
                ld      h, 0x6e; 37h ; '7'
                add     hl, hl
...
font	        incbin "_rus/intro_font.bin"
                ...
                ENT
ri_end:
	savebin "kissofmurder/exec_r.bin",ri_start,ri_end-ri_start
	ENDMODULE