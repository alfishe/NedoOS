...
notes_status                    ds 128,0                       
instrument                      ds 1,0 ;   uint8_t instrument; 
vibrato                         ds 1,0 ;   uint8_t vibrato;    
pitch_bend                      ds 2,0 ;   int16_t    pitch bend signed       midi_pitchbend
gm_rpn_coarse_tuning            ds 2,0  ; int16_t signed
gm_rpn_fine_tuning              ds 2,0  ; int16_t signed
_MIDI_RPN_TYPE                  ds 1,0
_MIDI_CTL_REGIST_PARM_NUM_MSB   ds 1,0 ;
_MIDI_CTL_REGIST_PARM_NUM_LSB   ds 1,0 ;                                                
_MIDI_CTL_MSB_DATA_ENTRY        ds 1,0 ;
_MIDI_CTL_LSB_DATA_ENTRY        ds 1,0 ;
gm_sustain                      ds 1,0
panpot                          ds 1,0 ;   uint8_t panpot;     
gm_rpn_pitch_bend_range         ds 2,0  ; int16_t signed
param_type                      ds 1,0 ; /* RPN/NRPN */
gm_volume                       ds 1,0
gm_expression                   ds 1,0
drum_channel                    ds 1,0 ;   bool    drum_channel;
               ends
...
reg_f_number    ds 1,0
reg_misc        ds 1,0
reg_lfo_vibrato ds 1,0
        ends


LOOP_TRACER_ENTRIES = 16
		struct LOOP_TRACER_DATA
loop_position	ds	4		
loops_count		ds	1
		ends


NOTE_TRACER_ENTRIES	= 12
		struct NOTE_TRACER_DATA
note				ds	1
wait_for_position		ds	2  ;4
		ends
...
midi_ch 						ds 1
midi_ch_addr					ds 2
key_offset						ds 1	;key,transposition ;"key" (transposition in semitones, 7-bit signed)
...
same_measure_position			ds 4
same_measure_mode				ds 1
same_measure_commands			ds 1    ;how many non FC commands was executed since previous FC command

current_loop					ds 1
loop_tracer						ds LOOP_TRACER_DATA*LOOP_TRACER_ENTRIES
note_tracer						ds NOTE_TRACER_DATA*NOTE_TRACER_ENTRIES
        ...
        struct MIDI_HEADER
ticks_per_qnote         ds 2 ;uint16_t		;Ticks per Quarter       ;sarx  timebase
...
tempo                   ds 1; tempo in bpm
play_bias               ds 1			;global transposition   int8_t  (signed)
number_of_tracks        ds 1       ;uint8_t  
detected_tracks         ds 1       ;uint8_t  
