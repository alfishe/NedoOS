...
#define TELBOOK_PATH "../ini/user.tel"
...
#define TELBOOK_ATTR_TITLE 0x67u
...
{
  return (unsigned char)(telbook_count + 1u);
}

static unsigned char telbook_is_file_item(unsigned char idx)
{
  return (idx >= telbook_count) ? 1u : 0u;
}
...
static void telbook_load(void)
{
  ...
  n = OS_READHANDLE((unsigned char *)buf, fp, (unsigned int)(sizeof(buf) - 1u));
  OS_CLOSEHANDLE(fp);
  if (n == 0u)
  {
    return;
  ...
  for (i = 0u; i <= n; i++)
  {
    if (buf[i] == 0 || buf[i] == '\r' || buf[i] == '\n')
    {
      ...
      line[len] = 0;
      if (telbook_parse_line(line, &tmp) && telbook_count < TELBOOK_MAX)
      {
        telbook[telbook_count] = tmp;
        telbook_count++;
      }
      line_start = i + 1u;
      if (buf[i] == '\r' && buf[i + 1u] == '\n')
...
static int telbook_save(void)
{
  ...
  {
    sprintf(line, "%s|%s|%u|%s\r\n",
            telbook[i].label,
            telbook[i].host,
            telbook[i].port,
            telbook[i].cp866 != 0u ? "866" : "437");
    out_len = (unsigned int)strlen(line);
    if (OS_WRITEHANDLE((unsigned char *)line, fp, out_len) != out_len)
    {
      OS_CLOSEHANDLE(fp);
      return 0;
    }
...
static void telbook_draw_title(void)
{
  telbook_draw_chip(1u, TELBOOK_TITLE_Y, 24u, TELBOOK_ATTR_TITLE, " atelnet address book ");
  telbook_draw_chip(56u, TELBOOK_TITLE_Y, 23u, TELBOOK_ATTR_TITLE, " user.tel            ");
}
...
static void telbook_build_row(char *line, unsigned char item_idx, unsigned char selected)
{
  ...
  if (telbook_is_file_item(item_idx))
  {
    telbook_paste(line, TB_ROW_O_LABEL, (unsigned char)(TB_INNER_W - TB_ROW_O_LABEL), "File browser");
    return;
  }

  telbook_field_text(TELBOOK_F_LABEL, &telbook[item_idx], text, sizeof(text));
  telbook_paste(line, TB_ROW_O_LABEL, TB_W_LABEL, text);
  telbook_field_text(TELBOOK_F_HOST, &telbook[item_idx], text, sizeof(text));
  telbook_paste(line, TB_ROW_O_HOST, TB_W_HOST, text);
  telbook_field_text(TELBOOK_F_PORT, &telbook[item_idx], text, sizeof(text));
  telbook_paste(line, TB_ROW_O_PORT, TB_W_PORT, text);
  telbook_field_text(TELBOOK_F_CP, &telbook[item_idx], text, sizeof(text));
  telbook_paste(line, TB_ROW_O_CP, TB_W_CP, text);
}
...
static void telbook_draw_edit_cursor(unsigned char y, unsigned char item_idx, const char *line)
{
  ...
  if (telbook_editing == 0u || item_idx != tel_sel || telbook_is_file_item(item_idx))
  {
    return;
  }
...
{
  char line[TB_INNER_W + 1u];
  unsigned char y;
  unsigned char selected;

  y = (unsigned char)(TELBOOK_LIST_Y + vis_y);
  selected = (item_idx == tel_sel) ? 1u : 0u;
  telbook_build_row(line, item_idx, selected);
  telbook_draw_inner_line(y, line, selected != 0u ? TELBOOK_ATTR_SEL : TELBOOK_ATTR_ROW);
  telbook_draw_edit_cursor(y, item_idx, line);
}
...
static void telbook_draw_header_row(void)
{
  ...
  telbook_build_header(line);
  telbook_draw_inner_line(TELBOOK_HDR_Y, line, TELBOOK_ATTR_HDR);
}
...
static void telbook_draw_list(void)
{
  ...
  for (q = 0u; q < TELBOOK_VIEW_H; q++)
  {
    ...
    else
    {
      ...
      blank[TB_INNER_W] = 0;
      telbook_draw_inner_line(y, blank, TELBOOK_ATTR_ROW);
    }
  }
}
...
{
  term_cls(0x07u);
  telbook_fill_rect(TELBOOK_WIN_X, TELBOOK_TITLE_Y, TELBOOK_WIN_W, 1u, TELBOOK_ATTR_ROW);
...
static void telbook_end_edit(unsigned char apply)
{
  ...
  if (apply != 0u && !telbook_is_file_item(tel_sel))
  {
...
static void telbook_delete_entry(void)
{
  ...
  if (telbook_is_file_item(tel_sel) || telbook_count == 0u)
  {
    return;
  }
  telbook_end_edit(1u);
...
static void telbook_file_stub(void)
{
  term_cls(0x07u);
  OS_SETCOLOR(0x0Fu);
  term_set_xy(4u, 10u);
...
{
  if (telbook_is_file_item(tel_sel))
  {
    telbook_editing = 0u;
    return 1u;
  }
                ...
                unsigned char *cp866, unsigned char *debug)
{
  ...
  for (;;)
  {
    ...
    else if (key == 13u)
    {
      ...
      else if (telbook[tel_sel].host[0] != 0)
      {
        telbook_end_edit(1u);
