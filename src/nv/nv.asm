...
editcmd_backspace
        call cmdcalctextaddr ;hl=addr, a=curcmdx
        or a
        jr z,editcmddirback;editcmdok ;нечего удал€ть
        dec a
        ld (curcmdx),a
        jp strdelch
...
;команда scratch - реально cmd scratch, запускает scratch по фону и выходит
        ;YIELD ;дать врем€ задаче cmd захватить фокус
        ;ld e,-1
        ;OS_SETGFX ;disable gfx, give focus
        WAITPID
execcmd_runfocusq
        ;YIELD ;дать врем€ системе передать фокус рандомной задаче
        ;ld b,25
;execcmd_waitchildredraw0
        ;push bc
        ;YIELD ;дать врем€ задаче scratch захватить фокус и перерисовать (но загрузить картинку и перерисовать не успеет)
        ;pop bc
...
editcmd_enter_runfile_nocom
        ...
        OS_SETSYSDRV ;TODO директори€ cmd
        ld hl,cmdprompt
        jp loadandrun ;nz=error, e=id
...
runfile_nocom_recodeext0
        ...
        OS_SETSYSDRV ;TODO директори€ nv
        ld hl,ext_filename
        call copy_to_fcb_filename
...
;load file in fcb from system current dir with parameters in tcmd, then set curpaneldir and run
	...
	call setcurpaneldir
        ...
        or a
        ret nz ;error
;dehl=номера страниц в 0000,4000,8000,c000 нового приложени€, b=id, a=error
        push bc ;b=id
        ld a,d
        SETPG32KHIGH
        push de
...
editcmd_4
	...
	and FATTRIB_DIR;#10
        ...
        OS_SETSYSDRV ;TODO директори€ texted
...
copybuf=0x4000 ;нельз€ 0xc000 - поверх какой-нибудь директории (а она используетс€ при копировании) ;0x8000 можно только после выставлени€ страницы там
        ;ds 4096;128
...
;pgfatfs=pagexor-9
;pgtrdosfs=pagexor-8
;pgkillable=pagexor-4 ;в 128K пам€ти, т.к. можно портить
