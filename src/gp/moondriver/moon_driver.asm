...
IDX_LOOP_ADDR: equ (seq_ch1_loop_addr - seq_work)
IDX_LOOP_BANK: equ (seq_ch1_loop_bank - seq_work)
IDX_NOT_LOOPED: equ (seq_ch1_not_looped - seq_work)
...
moon_seq_init:
	...
	ld	(seq_use_ch), a
	ld	(seq_cur_ch), a
	call	change_page3_bank  ; Page to Top
...
init_tone_fin:
	...
	ld	hl, S_LOOP_TABLE
	call	get_hl_table
	ld	(ix + IDX_LOOP_ADDR),hl

	ld	hl, S_LOOP_BANK
	call	get_a_table
	ld	(ix + IDX_LOOP_BANK),a
...
seq_loop:
	...
	ld	a,(ix + IDX_LOOP_BANK)
	ld	(ix + IDX_BANK),a
	call	change_page3_bank

	ld	hl,(ix + IDX_LOOP_ADDR)

	ld	(ix + IDX_NOT_LOOPED),0
	ret
...
seq_bank:
	...
	ld	a,(ix + IDX_LOOP_ADDR)
	cp	l
	ret	nz

	ld	a,(ix + IDX_LOOP_ADDR + 1)
	cp	h
	ret	nz
