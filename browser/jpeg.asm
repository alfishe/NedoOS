...
_thuffs=0xa800 ;размер=0x800? или меньше?
...
;pg2 = Cb -> B?
;pg5 = Cr -> R?
;по одним и тем же адресам, начиная с 0xc000
        CNVTO8C
;TODO fix
        ;jr $        LD HL,#C000            ;LD BC,#7FFD            ;LD DE,#7FDF        EXX         LD E,1        CALL SETPG        LD A,(MAXV8) ;высота полноценного блока        LD B,AYSLOOP8        PUSH BC        LD BC,(LSZX) ;TODO (curpicwid)?XSLOOP8        PUSH BC;/168900 CALLS;20t=1s!!!        EXX             ;LD A,#1B            ;OUT (C),A
            ld e,2
            call SETPG        LD A,(HL) ;CB
         ;xor a        LD (pCB8_),A            ;LD A,#1C            ;OUT (C),A            ld e,5
            call SETPG        LD A,(HL) ;CR         ;xor a       EXA             ;LD A,#19            ;OUT (C),A            ld e,1
        ...
        LD HL,G7170 ;CB tab (add to Y->G)       DEC H       LD A,(HL)       DEC H ;G7178/256 ;CB tab (add to Y->B) ;k<0?       LD L,(HL)       LD H,A        ADD HL,BC        ADD HL,DE ;de=таблица обрубания переполнения + Y        LD A,(HL) ;G?       LD (GC8_),A       LD HL,(pCB8_) ;G7170+ ;CB tab (add to Y->G) ;k>=0?        LD C,(HL)        INC H        LD B,(HL)       EX DE,HL ;hl=таблица обрубания переполнения + Y        ADD HL,BC       ; LD C,(HL)       ;LD B,GRF/256 ;таблица brightness/contract/dithering level       ;LD A,(BC)
         ld a,(hl) ;без контраста       EXA ;B?        EXX GC8_=$+1
        ;LD A,(GRF) ;таблица brightness/contract/dithering level        ;LD (HL),A         ld (hl),0 ;без контраста            ;LD A,#1B            ;OUT (C),A            ld e,2
            call SETPG       EXA ;B?        LD (HL),A            ;LD A,#1C            ;OUT (C),A            ld e,5
            call SETPGRC8_=$+1
        ;LD A,(GRF) ;таблица brightness/contract/dithering level        ;LD (HL),A         ld (hl),0 ;без контраста        INC HL        EXX         POP BC        DEC BC        LD A,B        OR C        JP NZ,XSLOOP8        POP BC        DEC B        JP NZ,YSLOOP8 ;на всю высоту полноценного блока        RET SHOWLNS
        ;jr $pCNVF   CALL CNVTO8C ;/EMPTY (for B/W)
;SHOWLNS
        LD A,(MAXV8) ;высота полноценного блока        LD B,A        LD HL,#C000        LD DE,(LSZX) ;ширина строки, округлённая вверх до полноценного блокаSHWLPP  PUSH BC        PUSH HL
        push DE        ;LD E,1        ;CALL SETPG       ;PUSH IX       CALL GET1 ;TODO при (pCNVF)==EMPTY вызывать B/W версию (или просто заранее чистить буфера строк pg4, pg5?)       ;POP IX        POP DE
...
render
        ...
                CALL YCCTAB        CALL GENMTAB       ;CALL MAKCTB      PUSH IY       CALL setsamplescalers       CALL maketrees;M18E08      POP IY                LD IX,PLTAB ;таблица отсечения переполнения        LD HL,PLTAB+512        LD B,LcNTBLP  LD (IX),B        INC IX        LD (HL),-1        DEC H        LD (HL),L        INC H        INC L        JR NZ,cNTBLP        if 1==0       LD A,(DITHP)       ADD A,100       LD DE,327       CALL MULDE_A       LD E,H
        LD HL,(pCNVF+1)        LD BC,EMPTY        XOR A        SBC HL,BC        LD HL,GRF        JR NZ,$+5        LD HL,PTAB        SUB E        LD D,AcorDGRF        LD A,(HL)        CP D        JR NC,$+3        XOR A        CP E        JR C,$+4        LD A,-1        LD (HL),A        INC L        JR NZ,corDGRF
        endif
...
;перебираем все слои?NXRS1   PUSH BC        LD E,(HL)        INC HL        LD D,(HL)        INC HL       PUSH HL       LD HL,(LPNT)       ADD HL,DE       LD (pLNADR+1),HL       POP HL        LD E,(HL)        INC HL        LD D,(HL)        INC HL       INC DE       LD (pQTBDC+1),DE      DEC DE
      dec DE       LD A,D       SUB Qorder/256       LD D,A       LD (pQTB+1),DE        LD E,(HL)        INC HL        LD D,(HL)        INC HL        LD C,(HL)        INC HL        LD B,(HL)        INC HL       PUSH HL       LD HL,#43C;16       ADD HL,DE       LD (pLHTB+2),HL       SBC HL,DE       ADD HL,BC       LD (pHHTB+2),HL       POP HL        LD A,(HL)       ADD A,-1       SBC A,A       AND IN3-IN2        LD (pXSCL+1),A        INC HL        LD A,(HL)        LD (pYSCL+1),A        INC HL        PUSH HL        CALL RDSAMP
pSMPPG  LD E,0 ;1/2/5        CALL SETPG ;TODO убратьpLNADR  LD HL,0pSFUNC  CALL 0 ;текущий адрес в таблице 4 функций ;по идее должно быть M1696A, M166AA, M1675A или EMPTY?        POP HL        POP BC        DJNZ NXRS1 ;следующий слой?
...
;onceP_QTAB        CALL RDWORD        DEC HL
        dec HL ;lenNQTB   PUSH HL        rdbyte        LD B,A        RRCA         RRCA         RRCA         RRCA         AND #0F       LD DE,#41       JR Z,$+4       LD E,#81       PUSH DE       LD C,A        LD A,B        AND #0F        CP 4        CALL NC,ERROR       ADD A,A       LD HL,VARS+#5B       ADD A,L       LD L,A        LD D,(HL)        DEC HL        LD E,(HL)        LD A,D        OR E        JR NZ,QTP       PUSH HL        LD DE,#80        CALL ALLDNP ;не портит bc!       EX DE,HL       POP HL       LD (HL),E       INC HL       LD (HL),DQTP     EX DE,HL        LD B,#40pQPRC   LD A,C       OR A       CALL NZ,RDBYTE        LD (HL),A        INC HL        rdbyte        LD (HL),A        INC HL        DJNZ pQPRC       POP DE       POP HL       OR A       SBC HL,DE       RET Z       RET C        JR NQTB 
        
;de=размер для очистки (реально очищается на 1 байт больше) и для сдвига MEMDN
;out: hl=старый (MEMDN), там произошла очистка
ALLDNP  CALL ALLDN        LD E,0        CALL SETPG        PUSH HL
        push DE
        push BC        LD E,L
        ld D,H        INC DEpCLR_=$+1        LD BC,0        LD (HL),0        LDIR         POP BC
...
SETPG
;TODO
;e=0/1 вручную, 1/2/5 по таблице (цветовые составляющие)
...
MEMDN   DW FREE ;указатель на какую-то память (для очистки в ALLDN) 
JF44    
        JP 0;JF48    
        JP 0;JF4C    
        JP 0;JF50    
...
XRES=curpicwid ;DW 0YRES=curpichgt ;DW 0RXRES   DW 0 ;ширина картинки в полноценных блокахRYRES   DW 0 ;высота картинки в полноценных блокахMAXV    DB 0MAXH    DB 0BLSZ    DW 0MAXH8   DW 0 ;ширина полноценного блока картинки ;по идее 8 или 16MAXV8   DW 0 ;высота полноценного блока картинки ;по идее 8 или 16LPNT    DW 0 ;указатель на графику полноценного блока (растёт от 0xc000 в строке блоков LSZX*MAXV8, по окончании строки блоков опять начинается с 0xc000)LSZX    DW 0 ;ширина картинки с округлением вверх до полноценного блокаLSZXM8  DW 0 
...
;TODO следующие таблицы убрать в 0x8000+        
...
VARS=VAR ;???
 ...
 ;+#4e = ??? только запись ;+#50-51 = ??? только запись ;+#52-53 = ??? только запись ;+#56-#57 = ??? результат вызова ALLDNP в jpgreadsizes;+#5A-#69=табл pQTB;+#6A-#79=табл pLHTB;+#7A-#89=табл pHHTB;+#C2=период вызова M18E7A (чанк M141FE);+#120=здесь он считается;+#110-#11F=табл.накопленных DC (+E0*2 байт)        ds 0x300 ;0x200?
...
SAMPLE  DS #80 ;блок 8x8 (по 2 байта), потом оттуда читаем через стек (подряд, один раз)SAMTAB  DS #80
