...
CALLBDOS        macro  ;don't use CALLBDOS or call 0x0005 directly!!!
        ...
        call 0x0005 ;c-CMD
        endm
OS_NETSOCKET        macro	;D=address family, E=socket type ; out: L=SOCKET(if L < 0 then A=error)
		ld l,0x01
        ld c,CMD_WIZNETOPEN
		CALLBDOS
        endm
OS_NETCONNECT        macro 	;A=SOCKET, DE=sockaddr ptr ; out: if HL < 0 then A=error
		ld l,0x03
        ld c,CMD_WIZNETOPEN
		CALLBDOS
        endm
OS_NETSHUTDOWN         macro ;A=SOCKET ; out: if HL < 0 then A=error
		ld l,0x02
        ld c,CMD_WIZNETOPEN
		CALLBDOS
        endm
OS_WIZNETCLOSE        macro 
		...
		CALLBDOS
        endm
OS_WIZNETREAD        macro ;A=SOCKET, de=buffer_ptr, HL=sizeof(buffer) ; out: HL=count if HL < 0 then A=error
        ld c,CMD_WIZNETREAD
		CALLBDOS
        endm
OS_WIZNETWRITE        macro ;A=SOCKET, de=buffer_ptr, HL=sizeof(buffer) ; out: HL=count if HL < 0 then A=error
