...
static uint16_t my_le16toh(uint16_t a)
{
	uint16_t result;

	result  =         (*(((uint8_t *)&a)+0));
	result += 256UL * (*(((uint8_t *)&a)+1));

	return result;
}

static uint32_t my_le32toh(uint32_t a)
{
	uint32_t result;

	result  =              (*(((uint8_t *)&a)+0));
	result +=      256UL * (*(((uint8_t *)&a)+1));
	result +=    65536UL * (*(((uint8_t *)&a)+2));
	result += 16777216UL * (*(((uint8_t *)&a)+3));

	return result;
}

static uint16_t my_htole16(uint16_t a)
{
	uint16_t result;

	*(((uint8_t *)&result)+0) = (a>>0) & 255UL;
	*(((uint8_t *)&result)+1) = (a>>8) & 255UL;

	return result;
}

static uint32_t my_htole32(uint32_t a)
{
	uint32_t result;

	*(((uint8_t *)&result)+0) = (a>> 0) & 255UL;
	*(((uint8_t *)&result)+1) = (a>> 8) & 255UL;
	*(((uint8_t *)&result)+2) = (a>>16) & 255UL;
	*(((uint8_t *)&result)+3) = (a>>24) & 255UL;
...
static void establish_params(struct device_info *info)
{
    ...
    if (!bs.secs_track)
        bs.secs_track = my_htole16(sec_per_track);
    if (!bs.heads)
        bs.heads = my_htole16(heads);
    bs.media = media;
...
static void setup_tables(void)
{
	...
	bs.boot_sign = my_htole16(BOOT_SIGN);
    } else {
    ...
    bs.reserved = my_htole16(reserved_sectors);
    if (verbose >= 2)
	printf("Using %d reserved sectors\n", reserved_sectors);
    bs.fats = (char)nr_fats;
    if (!atari_format || size_fat == 32)
	bs.hidden = my_htole32(hidden_sectors);
    else {
	/* In Atari format, hidden is a 16 bit field */
	uint16_t hidden = my_htole16(hidden_sectors);
    ...
    if (align_structures) {
        ...
        num_sectors = num_sectors / my_le16toh(bs.secs_track) * my_le16toh(bs.secs_track);
    }
	...
	case 12:
	    ...
	    bs.fat_length = my_htole16(fatlength12);
	...
	case 16:
	    ...
	    bs.fat_length = my_htole16(fatlength16);
	...
	case 32:
	    ...
	    bs.fat_length = my_htole16(0);
	    bs.fat32.fat32_length = my_htole32(fatlength32);
	...
	if (size_fat != 32)
	    bs.fat_length = my_htole16(fat_length);
	else {
	    bs.fat_length = 0;
	    bs.fat32.fat32_length = my_htole32(fat_length);
	}
    }
    ...
    if (fill_mbr_partition) {
        ...
        /* Partition type */
        if (my_le16toh(bs.heads) > 255 || my_le16toh(bs.secs_track) > 63) { /* CHS values are out of range for MBR, use LBA */
            if (size_fat != 32)
        ...
        else if (size_fat != 32 && num_sectors < my_le16toh(bs.secs_track) * my_le16toh(bs.heads) * 1024)
        ...
        if (my_le16toh(bs.heads) > 255 || my_le16toh(bs.secs_track) > 63 || num_sectors >= my_le16toh(bs.secs_track) * my_le16toh(bs.heads) * 1024) {
            /* If CHS address is too large use tuple (1023, 254, 63) */
            partition[5] = 254;
            partition[6] = 255;
            partition[7] = 255;
        } else {
            partition[5] = (num_sectors / my_le16toh(bs.secs_track)) % my_le16toh(bs.heads);
            partition[6] = ((1 + num_sectors % my_le16toh(bs.secs_track)) & 63) | (((num_sectors / (my_le16toh(bs.heads) * my_le16toh(bs.secs_track))) >> 8) * 64);
            partition[7] = (num_sectors / (my_le16toh(bs.heads) * my_le16toh(bs.secs_track))) & 255;
        }
	...
	bs.fat32.flags = my_htole16(0);
	bs.fat32.version[0] = 0;
	bs.fat32.version[1] = 0;
	bs.fat32.root_cluster = my_htole32(2);
	if (!info_sector)
	    info_sector = 1;
	bs.fat32.info_sector = my_htole16(info_sector);
	if (!backup_boot_set)
	...
	if (verbose >= 2)
	    printf("Using sector %d as backup boot sector (0 = none)\n",
		   backup_boot);
	bs.fat32.backup_boot = my_htole16(backup_boot);
	...
	printf("%s has %d head%s and %d sector%s per track,\n",
	       device_name, my_le16toh(bs.heads),
	       (my_le16toh(bs.heads) != 1) ? "s" : "", my_le16toh(bs.secs_track),
	       (my_le16toh(bs.secs_track) != 1) ? "s" : "");
	...
	if (ctime && ctime->tm_year >= 80 && ctime->tm_year <= 207) {
	    de->time = my_htole16((unsigned short)((ctime->tm_sec >> 1) +
						(ctime->tm_min << 5) +
						(ctime->tm_hour << 11)));
	    de->date = my_htole16((unsigned short)(ctime->tm_mday +
						((ctime->tm_mon + 1) << 5) +
						((ctime->tm_year - 80) << 9)));
	} else {
	    /* fallback to 1.1.1980 00:00:00 */
	    de->time = my_htole16(0);
	    de->date = my_htole16(1 + (1 << 5));
	}
	de->ctime_cs = 0;
	de->ctime = de->time;
	de->cdate = de->date;
	de->adate = de->date;
	de->starthi = my_htole16(0);
	de->start = my_htole16(0);
	de->size = my_htole32(0);
    }
	...
	info->signature = my_htole32(0x61417272);
	/* We've allocated cluster 2 for the root dir. */
	info->free_clusters = my_htole32(cluster_count - 1);
	info->next_cluster = my_htole32(2);

	/* Info sector also must have boot sign */
	*(uint16_t *) (info_sector_buffer + 0x1fe) = my_htole16(BOOT_SIGN);
    }
...
static void write_tables(void)
{
	...
	my_le32toh(bs.fat32.fat32_length) : my_le16toh(bs.fat_length);
    ...
    if (size_fat == 32) {
	seekto(my_le16toh(bs.fat32.info_sector) * sector_size, "info sector");
	writebuf(info_sector_buffer, 512, "info sector");
	if (backup_boot != 0) {
	    seekto(backup_boot * sector_size, "backup boot sector");
	    writebuf((char *)&bs, sizeof(struct msdos_boot_sector),
		     "backup boot sector");
	    if (backup_boot + my_le16toh(bs.fat32.info_sector) != my_le16toh(bs.fat32.info_sector) &&
		backup_boot + my_le16toh(bs.fat32.info_sector) < reserved_sectors) {
		seekto((backup_boot + my_le16toh(bs.fat32.info_sector)) * sector_size, "backup info sector");
