...
_TRD=0; SETVAR _TRD
_SCL=1; SETVAR _SCL
_HOB1=2; SETVAR _HOB1		;$??
_HOB2=3; SETVAR _HOB2		;!??
_SPG=4; SETVAR _SPG
_FDI=5; SETVAR _FDI
_TAP=6; SETVAR _TAP
_HOB=7; SETVAR _HOB
...
DELVAR=0 ;?
	...
	include "global_vars.asm"
	include "macros.asm"
	include "struct_mount.asm"
	include "fat_vars.asm"
VIRT_BITS=0x003a
...
TAB_MOUNTER	DW OPEN_MOUNT				;00 монтирование образа
	IF 1==0
		DW 0;RDWR_MOUNT				;01 чтение/запись примонтированных образов
		...
		DW STOR_NAMELOAD			;06 сохранение описателя файла загруженного в рамдиск
...
OPEN_MOUNT	
	LD A,(RREG_A)
OPEN_MOUNT_INT	
	LD (MOUNT_SYM),A			;номер монтирумого диска
		LD IYL,INTERNAL
...
IOPEN_MOUNT	LD DE,FILE_EXT
		...
		CALL ICOM_DEV
		DB _KOL_VOL ;2 запрос найденного
...
TXT_MOUNTER	DB "IMAGE.MNT",0				;имя для поиска текстового файла с указанием к монтированию TRD
ETXT_MOUNTER
...
MOUNT_RAMDISK	CALL CREATE4RAMDISK
		LD C,A
		LD A,(CPU2+VIRT_BITS)
		AND 0XF0
		LD B,A
		...
		edup ;ENDM
		AND B
		RET NZ				;если смонтирован образ, то рамдиск не монтируется
		LD A,B
		OR C
		JR WR_VIRT_BITS

CREATE4RAMDISK	LD H,VIRT_REAL_DRIVE
		PCALL READCMOS,ROM_RST82
		AND 3
		INC A
		LD B,A
...
cmd_end
	...
	savebin "mounter.com",cmd_begin,cmd_end-cmd_begin
